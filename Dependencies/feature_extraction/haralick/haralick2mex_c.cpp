...
void haralick2(double *image, double *haralicks, int ws, int dist, int graylevels, int background, int rows, int cols, int nharalicks) {
    ...
    double entropyval, energyval, inertiaval, idmval, contrastval,
    ...
    for (j=0; j<cols; j++) {
        for (i=0; i<rows; i++) {
            ...
            entropyval=0; energyval=0; inertiaval=0; contrastval=0; idmval=0;
            ...
            for (k=0; k<nnonzeros; k++) {
				...
				contrastval += (himhj[k] == k ? himhj[k]*himhj[k]*pnz[k] : 0);
			...
			haralicks[i+j*rows+13*rows*cols]=contrastval;
